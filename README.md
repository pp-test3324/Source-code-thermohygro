...
o	Core 0: Menangani koneksi WiFi, manajemen antrean (queue), Web Portal (WiFiManager), dan koneksi MQTT (MQTTS).
o	Core 1: Fokus untuk membaca sensor secara presisi, menghitung interpolasi kalibrasi, dan memperbarui layar OLED tanpa hambatan.
...
o	Pada saat pertama kali dinyalakan atau tidak ada koneksi WiFi, ESP32 akan memancarkan sinyal WiFi Access Point (AP) dengan format nama CALIB-ESP32-XXXXXX.
o	Hubungkan HP atau Laptop Anda ke WiFi tersebut.
o	Buka browser dan akses alamat IP 192.168.4.1 (biasanya akan terbuka otomatis melalui Captive Portal).
o	Atur WiFi rumah/kantor Anda dan masukkan parameter kalibrasi pada menu "Menu Kalibrasi (Suhu & RH)".
o	Anda juga dapat memicu mode ini secara manual kapan saja dengan menekan tombol yang terhubung pada GPIO 14.
4.	Pemantauan (Monitoring):
o	Setelah terkoneksi internet, alat akan otomatis mengunggah data pembacaan ke broker MQTT.
o	Layar OLED akan menampilkan Suhu terkalibrasi, Kelembapan terkalibrasi, dan status antrean data (Q:) jika data gagal terkirim dan disimpan sementara di buffer.
